...
length returns the numbers of items
...
square brackets allows you to return only portions of a vector
...
It's the same as `-(1:2). These commands run a sequence.`
...
dim returns the numbers and rows in a dataframe

```{r}
str(sleep)
```

str returns the size of the data set and the class of each column.
...
There is a significant relationship between brain weight and total
sleep.
